...
from LightGCN_no_cuda import LightGCN
import os  # //todo 修改
...
def seed_torch(seed=1029):  # //todo 修改
    random.seed(seed)
...
def data_loader(data, batch_size, n_usr, n_itm):
    ...
    pos_items = interected_items_df['item_idx'].apply(lambda x: random.choice(x)).values  # //todo 固定随机数

    neg_items = interected_items_df['item_idx'].apply(lambda x: sample_neg(x)).values  # //todo 固定随机数

    return list(users), list(pos_items), list(neg_items)
    torch.manual_seed(seed)  # //todo: 为CPU设置随机种子
...
if __name__ == '__main__':
    ...
    n_layers = 3  # 隐层数
    EPOCHS = 30  # 训练轮次
    BATCH_SIZE = 1024  # stochastic descent的mini-batch的大小
    DECAY = 0.0001
    K = 10
    seed_torch()
    loss_list_epoch = []  # 列表：储存loss值
    MF_loss_list_epoch = []  # 列表：储存MF-loss值
    reg_loss_list_epoch = []  # 列表：储存reg-loss值(正则化后）
    recall_list = []
    precision_list = []
    ndcg_list = []
    map_list = []
    ...
    test_df = test_df[(test_df['user'].isin(train_user)) & (test_df['item'].isin(train_item))]  # //todo:只取了train集
    valid_df = valid_df[(valid_df['user'].isin(train_user)) & (valid_df['item'].isin(train_item))]  # //todo:只取了train集
    ...
    optimizer = torch.optim.Adam(lightGCN.parameters(), lr=0.02)
    ...
    for epoch in tqdm(range(EPOCHS)):
        ...
        for batch_idx in range(n_batch):
            optimizer.zero_grad()  # 清空optimizer当前的计算图
            users, pos_items, neg_items = data_loader(train_df, BATCH_SIZE, n_users, n_items)
            users_emb, pos_emb, neg_emb, userEmb0, posEmb0, negEmb0 = lightGCN.forward(users, pos_items, neg_items)
            mf_loss, reg_loss = bpr_loss(users, users_emb, pos_emb, neg_emb, userEmb0, posEmb0,
                                         negEmb0)  # //todo:要去查询bpr loss
