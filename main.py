...
def get_metrics(user_embed_wts, item_embed_wts, n_users, n_items, train_data, test_data, k):
    ...
    if device == 'cuda':
        R_tensor = convert_to_sparse_tensor(r).cuda()
    ...
    if device == 'cuda':
        top_k_relevance_indices = torch.topk(relevance_score, k).indices.cuda()
    else:
...
if __name__ == '__main__':
    ...
    for j in range(4):
        for i in range(3):
            latent_dim = 64
            n_layers = j+1  # Try 1, 2 and 4 layers
            EPOCHS = 30
            BATCH_SIZE = 1024
            DECAY = 0.0001  # Try 0.001 and 0.00001
            K = 10
            seed_torch()
            ...
            print('Parameters: Latent Dim:{}, Hidden Layers:{}, EPOCHS:{}, Batch Size:{}, Decay:{}, Top K:{}'.format(
                latent_dim, n_layers, EPOCHS, BATCH_SIZE, DECAY, K))
            ...
            if device == 'cuda':
                lightGCN = LightGCN(train_df, n_users_train, n_items_train, n_layers, latent_dim, device).cuda()
            else:
